...
public interface MenuMapper extends IBaseMapper<MenuExample, Menu>{
	...
	public static final String JOIN_COLS = " m.`id`,m.`menu_name`,m.`menu_code`,m.`p_menu_code`,m.`sys_code`," +
			"m.`menu_url`,m.`menu_logo`,m.`level`,m.`sort`,m.`create_time`,m.`update_time`,m.`load_type` ";

	@Select("<script>"+
			" select "+JOIN_COLS+" from "+TABLE+ " m " +
			" left join role_menu_ref rm s on rm.menu_code = m.menu_code " +
